...
class MYCustomRolloutBuffer(BaseBuffer):
    """
    ...
    def reset(self) -> None:
        #Changed
        self.observations = [np.zeros((self.buffer_size, self.n_envs, *self.obs_shape), dtype=np.float32)] * self.buffer_size
    ...
    def _get_samples(
        ...
        data = (
            [self.observations[batch_ind] for batch_ind in batch_inds],
...
class MyCustomDictRolloutBuffer(MYCustomRolloutBuffer):
    """
    ...
    def __init__(
        ...
        super(MYCustomRolloutBuffer, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
    ...
    def reset(self) -> None:
        ...
        self.advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.generator_ready = False
        super(MYCustomRolloutBuffer, self).reset()
    ...
    def _get_samples(  # type: ignore[override]
        ...
        return DictRolloutBufferSamples(
            observations={key: self.to_torch([obs[batch_ind] for batch_ind in batch_inds]) for (key, obs) in self.observations.items()},
...
TLDict = Dict[str, List[th.Tensor]]
...
class MyCustomDictRolloutBufferSamples(NamedTuple):
    observations: TLDict
    actions: th.Tensor
